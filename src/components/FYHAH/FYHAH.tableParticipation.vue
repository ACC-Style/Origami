		...
		<template v-slot:content>
			<div class="font_copy p_4:md">
				...
				<section
					...
					v-for="(metric, index) in participation.data"
					:key="index"
					v-bind:class="{ 'bg_black-1': isEven(index) }"
				>
					<div class="flex_shrink w_30:md p_3:md p-t_3 p-x_3 font_0 font_1:md c_primary">
						{{ metric.label }}
						<a
					...
					<div class="font_bold w_30:md flex_shrink p_3:md p-x_3 font_1 font_0:md">{{ metric.value }}</div>
					<div class="c_primary-n4 font_n1 w_40:md flex_auto p-x_3 p_3:md">
						<p class="max-w_40 lh_2 m-t_0">{{ metric.associatedServices }}</p>
					</div>
				</section>
			</div>
		</template>
